...
if navigation == "User Guide":
    ...
    if guide_option == "Course Eligibility and Recommendation System":
        with st.expander("Steps for Course Eligibility and Recommendation System"):
            ...
            5. **View Eligible Courses**: Check the eligible courses and download them if needed.
            6. **View Recommended Courses**: Check the recommended courses and download them if needed.
            7. **View Combined Data**: Check the combined data and download it if needed.
            """)
        ...
        with st.expander("Steps for Quick Check"):
            ...
            4. **View Eligible Courses**: Check the eligible courses and download them if needed.
            5. **View Recommended Courses**: Check the recommended courses and download them if needed.
            6. **View Combined Data**: Check the combined data and download it if needed.
            """)
            ...
            if invalid_majors:
                ...
                if missing_majors:
                    ...
                    if combined_df_list:
                        ...
                        # Display the Eligible Courses DataFrame
                        if st.checkbox("Show Eligible Courses"):
                            st.header("Eligible Courses")
                            st.dataframe(combined_list)

                            # Download the DataFrame as CSV
                            st.header("Download Eligible Courses")
                            csv = combined_list.to_csv(index=False)
                            st.download_button(
                                label="Download data as CSV",
                                data=csv,
                                file_name='eligible_courses_list.csv',
                                mime='text/csv',
                            )

                        # Display the Recommended Courses DataFrame
                        if st.checkbox("Show Recommended Courses"):
                            st.header("Recommended Courses")
                            st.dataframe(recommended_courses)

                            # Download the DataFrame as CSV
                            st.header("Download Recommended Courses")
                            csv = recommended_courses.to_csv(index=False)
                            st.download_button(
                                label="Download data as CSV",
                                data=csv,
                                file_name='recommended_courses_list.csv',
                                mime='text/csv',
                            )

                        # Display the processed DataFrame
                        if st.checkbox("Show Combined Data"):
...
if navigation == "Quick Check":
    ...
    if st.button("Process Manual Input Data"):
        valid_input = True
        # Validate input
        ...
        if valid_input:
            ...
            st.session_state.combined_df = combined_df  # Save the dataframe in session state
...
if 'combined_list' in st.session_state:
    if st.checkbox("Show Eligible Courses Data"):
        st.header("Eligible Courses Data")
        st.dataframe(st.session_state.combined_list)

        # Download the DataFrame as CSV
        st.header("Download Combined List Data")
        csv = st.session_state.combined_list.to_csv(index=False)
        st.download_button(
            label="Download combined list data as CSV",
            data=csv,
            file_name='combined_list_cs.csv',
            mime='text/csv',
        )

if 'recommended_courses' in st.session_state:
    if st.checkbox("Show Recommended Courses Data"):
        st.header("Recommended Courses Data")
        st.dataframe(st.session_state.recommended_courses)

        # Download the DataFrame as CSV
        st.header("Download Recommended Courses Data")
        csv = st.session_state.recommended_courses.to_csv(index=False)
        st.download_button(
            label="Download recommended courses data as CSV",
            data=csv,
            file_name='recommended_courses_cs.csv',
            mime='text/csv',
        )

# Add checkbox to show combined data
